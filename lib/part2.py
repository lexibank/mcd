...
class CognateList(list):
    def __init__(self, doubt):
        list.__init__(self, [])
        self.doubt = doubt
...
def parse(raw_dir):
    ...
    cogp = re.compile(
        ...
        wordp +
        r'(,\s*' + wordp + ')*' + lcomment + r'?\.?')  # Gloss is optional (take over from reconstruction if missing)
    ...
    for i, row in enumerate(reader(raw_dir / 'MCD_2_2023-04-27.tsv', delimiter='\t'), start=1):
        ...
        for cog in cogs.split(';'):
            assert not cog.strip().startswith('?')
            ...
            if lg in bylang:
                ...
                bylang[lg] = forms
            cognates[LANGS[lg]] = CognateList(doubt)
        ...
        if cfs and cfs != '.':
            for t in cfs.split('@'):
                ...
                bylang = {}
                type_ = 'Cf.'
                m = re.match(r'\s*(Cf\.|See|Note|cf\.)( also)?', t)
                if not m:
    ...
    for i, row in enumerate(reader(raw_dir / 'MCD_2_loans.tsv', delimiter='\t'), start=1):
        cogs, comment = row[1:]
